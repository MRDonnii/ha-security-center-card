...
class HaSecurityCenterCard extends HTMLElement {
  ...
  _allEntities() {
    return [this.config.primary_alarm, this.config.secondary_alarm, this.config.open_count,
      ...(this.config.locks || []).map(x => x.entity), ...(this.config.contacts || []).map(x => x.entity),
  ...
  _render() {
    ...
    const locks = (this.config.locks || []).map(x => ({ ...x, state: this._s(x.entity)?.state }));
    const contacts = (this.config.contacts || []).map(x => ({ ...x, state: this._s(x.entity)?.state }));
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      .shell{--accent:${mode === "danger" ? "#ff626f" : mode === "home" ? "#ffbd59" : mode === "safe" ? "#5edbb0" : "#68b8ff"};position:relative;isolation:isolate;overflow:hidden;padding:28px;border:1px solid rgba(132,173,210,.22);border-radius:30px;background:radial-gradient(circle at 88% 3%,color-mix(in srgb,var(--accent) 14%,transparent),transparent 30%),linear-gradient(145deg,#0d1c2b,#07121f);box-shadow:0 24px 60px rgba(0,0,0,.3)}
      ...
      <div class="hero"><div class="visual"><div class="radar"><ha-icon icon="${info[2]}"></ha-icon></div></div><div class="hero-body"><div class="state-label">${this.config.primary_alarm_name} alarm</div><div class="state-title">${info[0]}</div><div class="state-copy">${info[1]}</div><div class="changed">Sidst ændret ${this._ago(primary?.last_changed)}</div><div class="modes">
        ${[["disarmed","Frakoblet","mdi:shield-lock-open-outline","#71bfff",this.config.actions.disarm],["armed_home","Hjemme","mdi:shield-home-outline","#ffbd59",this.config.actions.home],["armed_away","Ude","mdi:shield-lock-outline","#5edbb0",this.config.actions.away]].map(x=>`<div class="mode ${primary?.state===x[0]?"active":""} ${x[4]?"":"disabled"}" style="--tone:${x[3]}" ${x[4]?`data-script="${x[4]}"`:""}><ha-icon icon="${x[2]}"></ha-icon><b>${x[1]}</b></div>`).join("")}
      </div></div></div>
      <div class="section-title"><h2>Systemer</h2><span>Tryk for detaljer</span></div><div class="systems">
        ${[[this.config.primary_alarm,this.config.primary_alarm_name,info],...(this.config.secondary_alarm?[[this.config.secondary_alarm,this.config.secondary_alarm_name,secondaryInfo]]:[])].map(x=>`<div class="system" data-more="${x[0]}" style="--color:${x[2][3]==="danger"?"#ff626f":x[2][3]==="home"?"#ffbd59":x[2][3]==="safe"?"#5edbb0":"#68b8ff"}"><div class="system-icon"><ha-icon icon="${x[2][2]}"></ha-icon></div><div><b>${x[1]}</b><small>Ændret ${this._ago(this._s(x[0])?.last_changed)}</small></div><div class="system-state">${x[2][0]}</div></div>`).join("")}
        ...
        ${locks.map(x=>`<div class="access" style="--color:${x.state==='locked'?'#5edbb0':'#ffbd59'}"><ha-icon icon="${x.icon||'mdi:door-closed-lock'}"></ha-icon><b>${x.name}</b><span>${x.state==='locked'?'Låst':'Ulåst'}</span><div class="lock-action" data-lock="${x.entity}" data-state="${x.state}"><ha-icon icon="${x.state==='locked'?'mdi:lock-open-variant-outline':'mdi:lock-outline'}"></ha-icon></div></div>`).join("")}
        ${contacts.map(x=>`<div class="access" data-more="${x.entity}" style="--color:${x.state==='off'?'#5edbb0':'#ffbd59'}"><ha-icon icon="${x.icon||'mdi:lock-check-outline'}"></ha-icon><b>${x.name}</b><span>${x.state==='off'?'Sikret':'Ikke sikret'}</span></div>`).join("")}
    ...
    this.shadowRoot.querySelectorAll("[data-script]").forEach(el=>el.addEventListener("click",()=>this._service("script","turn_on",{entity_id:el.dataset.script})));
    this.shadowRoot.querySelectorAll("[data-more]").forEach(el=>el.addEventListener("click",()=>this._moreInfo(el.dataset.more)));
    this.shadowRoot.querySelectorAll("[data-lock]").forEach(el=>el.addEventListener("click",()=>this._service("lock",el.dataset.state==="locked"?"unlock":"lock",{entity_id:el.dataset.lock})));
    this.shadowRoot.querySelector("[data-nav]")?.addEventListener("click",e=>this._navigate(e.currentTarget.dataset.nav));
  }
}
